...
class LoginPage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnLogin.setOnClickListener {
            ...
            val courtType = findViewById<RadioButton>(binding.rgCourtType.checkedRadioButtonId).text.toString().uppercase()

            val dbquery = dbreference.child(binding.etRegistrarId.text.toString())
            dbquery.addListenerForSingleValueEvent(object : ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    if(snapshot.exists()){
                        ...
                        if (data != null) {
                            if(binding.etEmail.text.toString() == data.officialEmail && binding.etPassKEY.text.toString() == data.passKey
                        ...
                        Toast.makeText(this@LoginPage, "Registrar Id not valid", Toast.LENGTH_SHORT).show()
                    }
                }
